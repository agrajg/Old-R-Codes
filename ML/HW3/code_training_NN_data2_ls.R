...
ptm <- proc.time()
#################### DATA #############################
data <- read.table("hw3-nn-train-10000.dat", quote="\"", comment.char="")
...
while(min(check) >= tol)
{
  ...
  ################################################################
  ######################LINE SEARCH###############################
  w.ls.next <- w.cur - eta*d.k
  w.ls.cur <- w.cur
  eta <- c(seq(0.1, by=0, length.out = length(W0)))
  while(l.logit(w.ls.next)< l.logit(w.ls.cur))
  {
    eta <- eta + 0.1
    
    w.ls.cur <- w.ls.next
    w.ls.next <- w.ls.cur - eta*d.k
  }
...
####PLOTTING####
w.opt.data2 <- w.next
write.table(w.mat.iter, "w_mat_data2_ls.txt", sep="\t")
